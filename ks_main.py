...
def evaluate_exc(exc, rho, weights, hybrid):
    ...
    if alpha != 1.0:
        xc_energy -= np.einsum('pq,pq->', d, k, optimize=True) * (1.0 - alpha)
...
if __name__ == '__main__':
    ...
    mesh = 'close' ; functional = 'B3LYP'; DIIS = True ; DIIS_SIZE = 6

    #set hybrid parameters
    alpha = [1.00, 0.75, 0.80][functionals.index(functional)]  
    ...
    for cycle in range(cycles):
        ...
        rho = gga.evaluate_rho_gga(d, ao)

        #evaluate functional over mesh
        exc, vxc = gga.gga_functional(functional, rho, alpha)

        #evaluate energy
        xc_energy = evaluate_exc(exc, rho[0], weights, (alpha, k, d))
        ...
        #evaluate potential
        vxc = evaluate_vxc_gga(vxc, ao, weights, rho)
    ...
    out([eSCF, np.einsum('pq,pq->', d, (2.0*h_core), optimize=True), \
                np.einsum('pq,pq->', d, ( j), optimize=True), \
                evaluate_exc(exc, rho[0], weights, (alpha, k, d)), nuclear_repulsion(mol) ], 'final')
